/*
 * $Id: MPX5100a.c,v 1.8 2010/07/19 19:55:53 clivewebster Exp $
 *
 * Revision History
 * ================
 * $Log: MPX5100a.c,v $
...
static void __mpx5100a_init(SENSOR* sensor){
	...
	device->initial = (total + (SAMPLES >> 1)) / SAMPLES;
}
...
static void __mpx5100a_read(SENSOR* sensor){
	MPX5100A* device = (MPX5100A*)sensor;
	// read the sensor
	int16_t adc = a2dReadMv(device->adcPin);

	// make relative to initial setting
	int16_t zrel = adc - device->initial;

	// 45mV per kpA
	PRESSURE_TYPE val = zrel / 45;

	device->pressure.kPa = val;
}
