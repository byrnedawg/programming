/*
 * $Id: FAT.h,v 1.3 2010/06/14 19:16:07 clivewebster Exp $
 *
 * Revision History
 * ================
 * $Log: FAT.h,v $
...
typedef struct s_disk{
	const STORAGE_CLASS* class;		// The code for reading/writing sectors
...
/*-----------------------------------------------------------------
 Initialise a disk
 If the 'device' is an SD_CARD then 'class' should be '&c_sdCard_disk'
...
-----------------------------------------------------------------*/
static __inline__ uint32_t fileGetPos(const FILE* file){
	return file->filePos;
}
...
-----------------------------------------------------------------*/
static __inline__ uint32_t fileGetSize(const FILE* file){
