...
#define PWM_200Hz 0x27
#define PWM_30Hz 0xFC
...
ISR(ADC_vect)
{
	uint8_t temp;
	temp = ADCH ;
	Battery_Voltage = (56 * 255 / temp); // 56/ 1.1v = 255/5v
	
	if(Battery_Voltage < 132) //132/2.6V Low Battery Voltage divider R1 324Ohms R2 200 
 	{
		Low_Battery = true;
	//	PORTB = (0<<PB0);
	}
...
{	
	...
	OCR1A = 0x00;
	//OCR1B = 0x00;	
	OCR1C = PWM_200Hz;	//(1Mhz/prescaler)/(OCR1C+1)= PWM Frequency 	1M/128/39+1 = 200Hz	0x27 	1M/128/252+1 = 33Hz	0xFC
	TIMSK |= (1<<OCIE1A)|(1<<OCIE1B)|(0<<OCIE0A)|(0<<OCIE0B)|(1<<TOIE1)|(1<<TOIE0);		// Compare A and OVF Interupt enabled 
	TCNT1  = 0x00;
//	timer1Overflows = 0;
