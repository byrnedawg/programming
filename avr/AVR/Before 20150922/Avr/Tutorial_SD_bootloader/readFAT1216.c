...
unsigned char fat1216_init(void)
{
	...
	#endif
	{
		...
		//Load VBR
		disk_read_simp (buf, FATRegionStartSec);
...
{
	direntry_t *dir; //Pointer to a folder name
...
static void load_fat_sector(uint16_t sec)
{
	...
	{
		disk_read_simp (buf, FATRegionStartSec + sec);
...
void fat1216_readfilesector(uint16_t startcluster, uint16_t filesector)
{
	...
	secoffset = (uint8_t)filesector & (SectorsPerCluster-1); // SectorsPerCluster is always a power of 2!
	...
	if ((startcluster & 0xFFF0) == 0xFFF0) //FAT16 only has 16-bit cluster numbers
	...
	disk_read_simp (buf, templong + DataRegionStartSec + secoffset);
