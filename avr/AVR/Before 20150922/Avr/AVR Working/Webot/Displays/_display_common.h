/*
 * $Id: _display_common.h,v 1.3 2010/10/06 12:09:55 clivewebster Exp $
 *
 * Revision History
 * ================
...
typedef struct s_display {
	const struct PROGMEM c_display*	class;  // The constant definition of the display
...
#define MAKE_DISPLAY(class, columns, rows, writer) { class, columns, rows, writer, FALSE,FALSE,FALSE }
...
typedef struct PROGMEM c_display{
	...
	uint8_t	(*sendByte)(DISPLAY* display, uint8_t byte);
	void	(*hgraph)(DISPLAY* display,DISPLAY_COLUMN x,DISPLAY_LINE y,uint16_t pixels,uint8_t width);
	void	(*vgraph)(DISPLAY* display,DISPLAY_COLUMN x,DISPLAY_LINE y,uint16_t pixels,uint8_t width);
} DISPLAY_CLASS;

#define MAKE_DISPLAY_CLASS(init,cls,home,gotoXY,lineWrap,autoScroll,backlight,setBrightness,setContrast,send,hgraph,vgraph) { \
	init,cls,home,gotoXY,lineWrap,autoScroll,backlight,setBrightness,setContrast ,send,hgraph,vgraph \
}
