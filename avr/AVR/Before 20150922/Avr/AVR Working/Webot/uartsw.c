/*
 * $Id: uartsw.c,v 1.11 2010/07/01 23:56:37 clivewebster Exp $
 *
 * Revision History
 * ================
 * $Log: uartsw.c,v $
...
static void uartswTxBitService(const TimerCompare *channel, void* _uart)
{
	...
	if(uart->txBitNum)
	{
		...
		uint16_t next = (compareGetThreshold(channel) + uart->baudRateDiv);
		if(next >= top){
			next -= top;
		}
...
static void uartswRxStartService(const Timer *timer, void* _uart){
	...
	compareSetThreshold(rx, (capture + uart->baudRateDiv + uart->baudRateDiv/2) );
...
static void uartswRxBitService(const TimerCompare *channel, void* _uart){
	...
	compareSetThreshold(channel, (compareGetThreshold(channel) + uart->baudRateDiv));
...
void __uartswInit(UART* _uart, BAUD_RATE baud){
	...
	if(uart->_uart_.rx_pin!=null || uart->_uart_.tx_pin!=null){
		...
		if(uart->_uart_.tx_pin!=null){
			...
			compareAttach(timerGetCompare(timer,0),&uartswTxBitService,0,uart);
		}
...
calcBaud:
	uart->baudRateDiv = (uint16_t)(  (cpu_speed+(baudrate/2L)) / (prescale * baudrate * 1L)  );

	if(uart->baudRateDiv > top){
		uart->baudRateDiv = top;
		if(isMyTimer){
...
void __uartswStartXmit(UART* _uart, uint8_t data){
	...
	if(uart->_uart_.tx_pin){
		...
		uint16_t next = (start + uart->baudRateDiv);
		if(next >= top){
			next -= top;
		}

		compareSetThreshold(timerGetCompare(timer,0), next);
	}else{
