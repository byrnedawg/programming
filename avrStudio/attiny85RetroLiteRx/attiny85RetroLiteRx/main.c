/*
 * attiny85RetroLiteRx.c
 *
 * Created: 2/1/2017 5:57:36 PM
...
volatile bool validRecieved; // true if the push button is being help down
...
ISR(TIMER1_COMPA_vect) // timer 1 compare A interrupt
{
	...
	if(timer1Overflows % 10 == 0) // 2 per second 2 * 5 = 5 seconds
	{
		output_toggle(PORTB, DataOut);
	}
	*/
}
...
void PortInit(void) // initialize port B inputs and outputs
{
	//												   ______
	//	(PCINT5/~RESET/ADC0/dW)			Reset	--PB5 |		 |VCC--	5V
	//	(PCINT3/XTAL1/CLKI/~OC1B/ADC3)	TxEn    --PB3 |		 |PB2--	Switch1	(SCK/USCK/SCL/ADC1/T0/INT0/PCINT2)
	//	(PCINT4/XTAL2/CLKO/OC1B/ADC2)	TxData  --PB4 |		 |PB1--	Divider	(MISO/DO/AIN1/OC0B/OC1A/PCINT1)
	...
	PORTB = 0x00;
	DDRB = 0x00;
	set_input(DDRB, DataIn);
	set_input(DDRB, VT);
	set_output(DDRB, DataOut);
...
{
	validRecieved = false; // button is not pushed down
	PCMSK |= (0<<PCINT5)|(0<<PCINT4)|(1<<PCINT3)|(1<<PCINT2)|(0<<PCINT1)|(0<<PCINT0); // pin change mask PCINT5 PCINT4 PCINT3 PCINT2 PCINT1 PCINT0
	GIMSK |= (0<<INT0)|(1<<PCIE); // enable PCINT interrupt
	GIFR |= (0<< INTF0)|(0<< PCIF);
...
void powerOptions(void)
{
	PRR |= (0<<PRTIM1)|(0<<PRTIM0)|(0<<PRUSI)|(0 <<PRADC);
	MCUSR |= (0<<WDRF)|(0<<BORF)|(0<<EXTRF)|(0<<PORF);
	WDTCR |= (0<<WDIF)|(0<<WDIE)|(0<<WDP3)|(0<<WDCE)|(0<<WDE)|(0<<WDP2)|(0<<WDP1)|(0<<WDP0);
...
{
	TCCR1 |= (1<<CTC1)|(0<<PWM1A)|(0<<COM1A1)|(0<<COM1A0)|(1<<CS13)|(1<<CS12)|(1<<CS11)|(1<<CS10); // PWM1A Disabled, Timer/Counter Comparator A disconnected from output pin OC1A, prescale= CK/128 right now was CK/8 1000b ~ 1100b
	...
	OCR1C = 0xF3;//PWM_200Hz;	// OCR1C = Governs PWM Frequency :: (1Mhz/prescaler)/(OCR1C+1)= PWM Frequency :: 1M/128/39+1 = 200Hz	0x27 1M/128/252+1 = 33Hz	0xFC
	TIMSK |= (1<<OCIE1A)|(0<<OCIE1B)|(0<<OCIE0A)|(0<<OCIE0B)|(0<<TOIE1)|(0<<TOIE0);	// Compare A and OVF Interrupt enabled
	TCNT1 = 0x00; // Start Timer CTC 0x1E83
